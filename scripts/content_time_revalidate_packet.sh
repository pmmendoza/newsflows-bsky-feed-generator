...
cmd_apply() {
  ...
  if (( nb > 0 )); then
    ...
    local perbatch; perbatch=$(node -e 'const j=JSON.parse(require("fs").readFileSync(process.argv[1]));const b=(j.revalidation&&j.revalidation.batches)||[];const [r,m,p,fl]=process.argv.slice(2).map(Number);let worst=0,worstRatio=0,fail=0,lines=[];for(const x of b){if(typeof x.wal_bytes!=="number"){lines.push(`batch=${x.batch} wal_bytes=missing`);fail++;continue;}const ms=x.elapsed_ms||0;const adj=Math.max(0,x.wal_bytes-Math.round(r*ms/1000));const ceil=Math.max(fl,Math.round(m*r*(ms/1000+p)));const ok=adj<=ceil;if(!ok)fail++;const ratio=ceil?adj/ceil:0;if(ratio>worstRatio){worstRatio=ratio;worst=adj;}lines.push(`batch=${x.batch} elapsed_ms=${ms} lsn_advance=${x.wal_bytes} attributed=${adj} ceiling=${ceil} ratio=${ratio.toFixed(2)} ${ok?"ok":"BREACH"}`);}console.log(JSON.stringify({fail,worst,worstRatio:worstRatio.toFixed(2),lines}))' "$f" "$wps" "$CEIL_WAL_BASELINE_MULTIPLE" "$PAUSE_SECONDS" "$CEIL_WAL_FLOOR_BYTES")
    ...
    { echo "batches=$nb exit=$rc updated=$(jsonq "$f" revalidation.updated) complete=$(jsonq "$f" revalidation.complete) invocation_seconds=$dur"; echo "delta_wal_bytes=$dw delta_relation_bytes=$dr delta_dead_tuples=$dd"; echo "control_file=$(basename "$(latest_control)") control_rate_per_s wal=$wps relation=$rps dead=$dps (clamped at 0)"; echo "per_batch_relation_minus_control=$pr per_batch_dead_minus_control=$pd (invocation deltas minus baseline x duration, / batches)"; echo "wal_rule=per batch: attributed (LSN advance - baseline x elapsed) <= max(floor $CEIL_WAL_FLOOR_BYTES, $CEIL_WAL_BASELINE_MULTIPLE x baseline $wps B/s x (elapsed + ${PAUSE_SECONDS}s)); cluster-wide LSN, concurrent writers included then subtracted"; echo "$wal_lines"; echo "wal_batches_failing=$wal_fail wal_worst_attributed=$wal_worst wal_worst_ratio=$wal_ratio"; echo "ceilings relation<=$CEIL_REL_BYTES dead<=$dead_ceiling (=2 x updated rows per batch)"; echo "verdict=$verdict"; } | emit "ceiling-$g-$label.txt"
  else
